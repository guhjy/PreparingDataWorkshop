...
print(WVPlots::ROCPlot(dTrainV,'pred',yName,
                       paste('vtreat prune model train')))
dTestV$pred <- predict(modelV,newdata=dTestV,type='response')
print(errorRate(dTestV$pred,dTestV[[yName]]))
print(WVPlots::ROCPlot(dTestV,'pred',yName,
                       paste('vtreat prune model test')))
print(WVPlots::DoubleDensityPlot(dTestV,'pred',yName,
                       paste('vtreat prune model test')))
```

Or we can try a "cross method" which approximates the Jacknife methods by building a special training
frame that has not been produced by the returned vtreat treatments (requires vtreat 0.5.22 or newer for 
)
...
crossExpmt <- vtreat::mkCrossFrameCExperiment(dTrain,vars,yName,TRUE,
                                        rareSig=1.0,
                                        smFactor=5.0,
                                        parallelCluster=cl)
treatments <- crossExpmt$treatments
dTrainV <- crossExpmt$crossFrame
print(treatments$scoreFrame)
varsV <- treatments$scoreFrame$varName[treatments$scoreFrame$sig<=0.05]
dTestV <- vtreat::prepare(treatments,dTest,pruneSig=c(),
...
The point is, many variations work well.
