...
parallelCl = parallel::makeCluster(parallel::detectCores())
...
permutation_test = function(dataf, nperm) {
  mkWorker = function() {
    bindToEnv(objNames=c('glm_significance','performance_eval','get_deviance'))
    nrows = dim(dataf)[1]
    y = dataf$y
  ...
  doperm = mkWorker()
  prlist = parallel::parLapply(parallelCl,seq_len(nperm), doperm)
  as.data.frame(dplyr::bind_rows(prlist),stringsAsFactors=FALSE)
}
